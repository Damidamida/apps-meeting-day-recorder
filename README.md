...
автоматически. Bundled FFmpeg проверяется внутри мастера.
...
Установите и запустите OBS Studio. В OBS откройте настройки WebSocket через меню `Инструменты`, включите сервер WebSocket и задайте пароль. Затем укажите локальные настройки подключения в `config.yaml`:
