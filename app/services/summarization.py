...
- не выдумывай факты;
- если у встречи отсутствует summary, явно укажи это в разделе "По встречам";
    ...
    def summarize_day(
        ...
        try:
            ...
            return {
                ...
                "day_summary_error": "Не удалось подготовить итоги дня через OpenAI.",
            }
...
def day_summary_message(metadata: dict[str, Any]) -> str:
    ...
    if status == "openai_unavailable":
        return "Итоги дня не подготовлены: OpenAI API key не найден."
    if status == "failed":
        return "Итоги дня не подготовлены: не удалось подготовить итоги дня через OpenAI."
...
def _day_summary_input(current_summary: str, meeting_summaries: list[dict[str, Any]]) -> str:
    ...
    for index, item in enumerate(meeting_summaries, start=1):
        ...
        if summary_text:
            ...
            body = "Summary отсутствует у этой встречи."
        sections.append(
            f"## {index}. {title}\n"
            f"- Время: {started_at}\n"
