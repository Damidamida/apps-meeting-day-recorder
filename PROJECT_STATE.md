...
В ветке `codex/workday-dashboard-prototype` готовится следующая UI-доработка этапа 8: вкладка `Рабочий день` приводится ближе к прототипу. Действия переносятся внутрь смысловых блоков, активный созвон получает таймер, встречи за день отображаются карточками-аккордеонами, а pipeline показывается внутри раскрытой карточки конкретной встречи. Старый отдельный блок `Действия` убирается. Логика OBS, FFmpeg, Whisper/faster-whisper, Summary generation и lifecycle встреч не меняется.

В ветке `codex/review-screen-prototype` готовится следующая stacked-доработка этапа 8 поверх PR по `Рабочему дню`: вкладка `Ревью` приводится ближе к прототипу. Встречи отображаются карточками, у выбранной встречи остаются только вкладки `Итоги встречи` и `Транскрипт`, transcript показывается read-only, а `Итоги дня` вынесены в отдельный блок и появляются только при наличии `00_day_summary_draft.md`.

В ветке `codex/settings-screen-prototype` готовится следующая stacked-доработка этапа 8 поверх PR по `Ревью`: placeholder вкладки `Настройки` заменяется рабочей формой для локального `config.yaml`. В UI можно редактировать storage root, OBS WebSocket, transcription backend/model/language/device/compute type/command, summary enabled/model/API key env/base URL/env file/timeouts/chunk size и заготовку темы. После сохранения приложение сообщает, что для OBS/transcription/summary/storage нужен перезапуск.

В ветке `codex/archive-help-ui-polish` готовится финальная stacked-доработка текущего UI-пакета этапа 8: вкладка `Архив` оформляется как честная заглушка будущего read-only просмотра прошлых дней, а `Справка` переводится в карточный формат с основным сценарием, local-first правилами и описанием сервисов.

Договоренность по задачам дня: функционал задач дня скрывается из UI, но существующая файловая логика `00_tasks_draft.md` / `00_tasks_final.md` не удаляется в этом PR, чтобы не ломать старые данные. Новая AI-агрегация итогов дня из итогов всех встреч не входит в текущую UI-доработку и должна быть выделена в будущий функциональный PR. В текущем UI итог дня будет опираться на существующий файл `00_day_summary_draft.md`.
...
Проверить финальный stacked PR текущего UI-пакета: убедиться, что `Архив` явно помечен как будущая read-only заглушка, `Справка` оформлена карточками и весь UI-пакет PR #29–#32 можно проверять по порядку. Этап 9 не начинать до принятия доработок этапа 8.
