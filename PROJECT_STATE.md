...
В ветке `codex/dark-theme-ui` реализуется новая доработка этапа 8: полноценная темная тема основного UI и отдельный выбор темы для floating control. Тема основного окна применяется сразу после `Сохранить настройки`; floating control может наследовать тему приложения или использовать отдельную светлую/темную тему. Изменение касается только UI-слоя и конфигурации интерфейса. Логика OBS, FFmpeg, Whisper/faster-whisper, Summary generation, lifecycle хранения и локальных данных не меняется.
...
Проверить PR с веткой `codex/dark-theme-ui`: переключение темы основного окна в `Настройки`, мгновенное применение после сохранения, отдельный выбор темы floating control и визуальную читаемость светлой и темной темы. Этап 9 не начинать до принятия доработок этапа 8.
...
- Ветка `codex/dark-theme-ui`: доработка этапа 8 с полноценной темной темой UI. Добавлен `ui.theme` для основного окна и `ui.floating_theme` для floating control; настройки темы сохраняются в локальный `config.yaml`, основная тема применяется сразу после сохранения, floating control может наследовать тему приложения или использовать отдельную светлую/темную тему. Темная палитра применена к основному окну, карточкам, настройкам, ревью, модальному окну старта встречи и floating control. Логика OBS, FFmpeg, Whisper/faster-whisper, Summary generation, lifecycle хранения и локальных данных не менялась. Этап 8 остается `На проверке`, этап 9 не начинался. Проверки: `python -m pytest` — `91 passed`; `python -m compileall -q app` — успешно.
