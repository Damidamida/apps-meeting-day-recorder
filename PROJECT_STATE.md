...
В ветке `codex/floating-meeting-control` реализуется новая доработка этапа 8: плавающий desktop control для быстрого старта рабочего дня, старта созвона, ввода названия встречи и завершения активного созвона с обязательным подтверждением. Floating control появляется автоматически при запуске приложения, может скрываться/показываться из основного окна, перетаскивается мышкой, при активном созвоне показывает тот же счетчик длительности, что и блок `Активный созвон`, и использует существующую lifecycle-логику `MainWindow` без отдельного `StorageService` и без дублирования OBS/FFmpeg/Whisper/Summary pipeline. Фоновые обработки показываются кратко и не блокируют старт следующего созвона.
...
Проверить PR с веткой `codex/floating-meeting-control`: автоматический показ плавающей кнопки, перетаскивание, старт рабочего дня, старт созвона с вводом названия, счетчик длительности активного созвона, обязательное подтверждение завершения созвона, скрытие/показ из основного окна и отсутствие блокировки нового созвона фоновым pipeline. Этап 9 не начинать до принятия доработок этапа 8.
