...
Проверить PR с переработкой вкладки `Рабочий день`: убедиться, что действия находятся в смысловых блоках, таймер активного созвона работает, встречи раскрываются как карточки-аккордеоны, pipeline отображается для выбранной встречи, а кнопки открытия папки встречи и папки дня находятся в блоке `Встречи за день`. После этого продолжить тот же большой UI-этап следующими PR для `Ревью`, `Настройки`, `Архив` и `Справка`. Этап 9 не начинать до принятия доработок этапа 8.
