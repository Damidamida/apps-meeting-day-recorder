...
Проверить stacked PR с рабочей вкладкой `Настройки`: убедиться, что форма сохраняет локальный `config.yaml`, секреты не выводятся открытым текстом, а приложение понятно сообщает о необходимости перезапуска для применения сервисных настроек. После этого продолжить тот же большой UI-этап следующим PR для `Архив` и `Справка`. Этап 9 не начинать до принятия доработок этапа 8.
