...
Этапы 1–4 завершены и приняты пользователем. Этап 5 — FFmpeg audio extraction остается в статусе `На проверке`. Промежуточный этап 5.1 — Windows launcher реализован в ветке `codex/windows-launcher`, PR пока не назначен, и находится в статусе `На проверке`.
