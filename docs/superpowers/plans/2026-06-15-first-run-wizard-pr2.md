...
- OBS показывает поля `Адрес WebSocket`, `Порт WebSocket` и скрытое поле `Пароль WebSocket`; значения сохраняются только после успешного WebSocket-check;
...
- На шаге `OBS` доступны поля адреса, порта и скрытого пароля WebSocket; проверка использует введенные значения и сохраняет их только после успеха.
